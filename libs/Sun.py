...
class Sun:
    ...
    def getSunriseTime( self, coords, UTC = False):
        return self.getSSTime(coords, UTC, True)

    def getSunsetTime( self, coords, UTC = False ):
        return self.getSSTime(coords, UTC, False)

    def getSSTime( self, coords, UTC = False, Sunrise = True ):
        data = self.calcSunTime( coords, Sunrise )
        if (not UTC):
